        ...
        for pos in pos_range:
            ...
            if np.abs((pos + 3.5) / vel) < 12:
                ...
                for num in range(n_sim):
                    ...
                    if not os.path.exists(collision_info_path) and ped_sfm.thr_gap > 0 and i_plot > 0:
...
def anaylze_particular_one(pos, vel, num, datetime, ctrl):
    init_state = [pos, vel]

    # for num in range(n_sim):

    # read data
    pickle_path = f'D:\\vpi_crossing_result\\{datetime}_sim_result_{ctrl}_{predict_method}\\' \
                  f'pos {int(init_state[0]):3d} ' \
                  f'vel {int(init_state[1]):2d} ' \
                  f'itr {num:3d}.p'
    layout, ped, veh_ego, ped_sfm, predictor, con_ego = pickle.load(open(pickle_path, "rb"))
...
if __name__ == '__main__':
    ...
    # anaylze_particular_one(pos=-25, vel=6, num=9, datetime='now', ctrl='mpc')
