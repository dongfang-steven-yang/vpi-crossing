        ...
        if problem.status == cp.OPTIMAL or problem.status == cp.OPTIMAL_INACCURATE:
            ...
            if self.verbose:
                ...
                for i in range(20):
                    ...
                    print(f'at step {i+1} dist to predicted obs = {obss_closest[i+1]:.4f} - {xx[0][0]:.4f} = {obss_closest[i+1] - xx[0][0]:.4f}')
    ...
    def _get_closest_dist(self, pred, pos_veh_front):
        ...
        if pos_veh_front < pred[0][0]: # before the vehicle reach the crossing point
            for i in range(len(pred)):
                ...
                # todo: modify the W_lane
                if 0 < lat_min < 3.2 or 0 < lat_max < 3.2 or (0 < lat_min and lat_max < 3.2) or (lat_min < 0 and 3.2< lat_max):
...
class PIDController:
    ...
    def generate_control(self, ref_speed, obj_pred):
        ...
        if min_target < 1000:
            d_dec = min_target - self.vehicle.state[0] - self.d_safe
